use clap::{crate_authors, crate_description, crate_version, crate_name, Arg, App};
use eyre::{eyre, Result};
use webdriver_install::Driver;
use std::path::PathBuf;

pub fn run() -> Result<PathBuf> {
    let supported_drivers: &[&str] = &vec![Driver::Chrome, Driver::Gecko].iter().map(Driver::as_str).collect::<Vec<&str>>();
